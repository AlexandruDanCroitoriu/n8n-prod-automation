export async function POST(
  ...
  const res = await fetch(`${baseUrl}/sp_add_product_links`, {
    method: "POST",
    headers: {
      Authorization: `Basic ${credentials}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ product_id: Number(id), links: body.links }),
  });

  if (!res.ok) {
    const text = await res.text();
    console.error("[product-links-add] webhook error", res.status, text);
