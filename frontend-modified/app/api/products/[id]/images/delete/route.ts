...
//   const baseUrl = process.env.WEBHOOK_URL_DEBUG;
  ...
  const res = await fetch(`${baseUrl}/sp_product_images_delete`, {
    method: "POST",
    headers: {
      Authorization: `Basic ${credentials}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ product_id: Number(id), images: body.images }),
  });

  if (!res.ok) {
    const text = await res.text();
    console.error("[product-images-delete] webhook error", res.status, text);
